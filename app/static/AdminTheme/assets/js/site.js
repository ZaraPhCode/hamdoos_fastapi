...
SinglePage.LoadModal = function () {
    ...
    //    },
    //    error: function () {
    //        $("#Tasks").html('<h2 style="color: red;">دریافت با خطا مواجه شد</h2>');
    //    },
    //    complete: function () {
    //        $("#TaskBody").unblock();
    //    },
...
$(document).ready(function () {
    ...
    let thirdSlashIndex = currentUrl.indexOf('/', currentUrl.indexOf('/', currentUrl.indexOf('/') + 1) + 1);
    if (thirdSlashIndex !== -1) {
        currentUrl = currentUrl.substring(0, thirdSlashIndex);
    }
    $("#sidebar-menu a").each(function () {
        if ($(this).attr("href")) {
            if (currentUrl == $(this).attr("href").toLowerCase()) {
                if (this.parentNode && this.parentNode.parentNode) {
                    var a = $(this.parentNode.parentNode).prev();
                    if (a) {
                        a.click();
                    }
                }
                ...
                //while (currentElem) {
                //    var test = currentElem.tagName;
                //    if (currentElem.tagName === "A" && currentElem.classList.contains("waves-effect")) {
                //        ancestor = currentElem.parentNode;
                //        $(ancestor).click();
                //        //ancestor = currentElem; // Found an <a> with the class "waves-effect"
                //    }
